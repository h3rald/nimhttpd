...
const
  ...
  hs = "nimhttpd"
  hs_file = "nimhttpd.nim"
  zip = "zip -X"

proc filename_for(os: string, arch: string): string =
  return "nimhttpd" & "_v" & version & "_" & os & "_" & arch & ".zip"

task "windows-x64-build", "Build NimHTTPd for Windows (x64)":
  direshell compile, windows_x64, hs_file

task "linux-x86-build", "Build NimHTTPd for Linux (x86)":
  direshell compile, linux_x86,  hs_file
  
task "linux-x64-build", "Build NimHTTPd for Linux (x64)":
  direshell compile, linux_x64,  hs_file
  
task "linux-arm-build", "Build NimHTTPd for Linux (ARM)":
  direshell compile, linux_arm,  hs_file
  
task "macosx-x64-build", "Build NimHTTPd for Mac OS X (x64)":
